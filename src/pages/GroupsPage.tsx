...
export default function GroupsPage() {
  ...
  const { data: groups, isLoading } = trpc.wordGroup.list.useQuery();
      ...
      <main className="max-w-3xl mx-auto px-4 py-6 pb-24">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">
              共 {groups?.length ?? 0} 个分组
            </p>
          </div>
        </div>

        {/* Create/Edit Form */}
        <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-100 p-4 space-y-3 mb-6">
          <div>
        ...
        {isLoading ? (
          ...
          <div className="space-y-2">
            <p className="text-xs text-gray-400 font-medium px-1">已有分组（拖拽排序）</p>
            {groups.map((group, index) => {
              ...
              return (
                <div
                  ...
                  className={`flex items-center gap-1.5 p-3 rounded-xl border transition-all cursor-grab active:cursor-grabbing ${
                    dragIndex === index
